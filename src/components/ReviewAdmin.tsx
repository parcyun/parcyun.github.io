...
export default function ReviewAdmin() {
  ...
  return <div className="cs-data cs-review-manager"><div className="cs-data-top"><p><b>{items.length}</b>개 리뷰</p><button onClick={load}>새로고침</button></div>{status && <p className="cs-status">{status}</p>}<div className="cs-data-list">{items.map((item) => <article className="cs-data-row" key={item.id}><div className="cs-data-main"><small>{'★'.repeat(item.rating)} · <span className={`service-tag service-${item.service_key}`}>{serviceLabels[item.service_key]}</span></small><p>{item.body}</p><span>공감 {item.like_count} · {new Date(item.created_at).toLocaleString('ko-KR')}</span></div><div className="cs-row-actions">{editingServiceId === item.id ? <select aria-label="서비스 태그" value={item.service_key} disabled={busyId === item.id} onChange={(event) => updateService(item, event.target.value as ServiceKey)}>{Object.entries(serviceLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select> : <button onClick={() => setEditingServiceId(item.id)}>태그 수정</button>}{item.status === 'pending' && <><button disabled={busyId === item.id} onClick={() => review(item, 'published')}>승인</button><button disabled={busyId === item.id} onClick={() => review(item, 'rejected')}>거절</button></>}{item.status === 'published' && <span className="is-published">published</span>}{item.status === 'rejected' && <span className="is-rejected">rejected</span>}{item.status !== 'pending' && <button disabled={busyId === item.id} onClick={() => review(item, 'pending')}>보류</button>}<button className="danger" disabled={busyId === item.id} onClick={() => remove(item)}>삭제</button></div></article>)}</div></div>;
}
