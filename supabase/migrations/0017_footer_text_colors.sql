-- Independent shared-footer primary, foreground, and muted text colors.
...
declare
  allowed text[] := array['color','foregroundColor','mutedColor','backgroundColor','fontFamily','fontSize','fontWeight','lineHeight','letterSpacing','padding','margin','borderRadius','borderColor','borderWidth','opacity','display'];
  item record;
  value_text text;
begin
  ...
  for item in select key, value from jsonb_each(p_values) loop
    ...
    if item.key in ('color', 'foregroundColor', 'mutedColor', 'backgroundColor', 'borderColor') and value_text !~* '^(#[0-9a-fA-F]{3}|#[0-9a-fA-F]{4}|#[0-9a-fA-F]{6}|#[0-9a-fA-F]{8}|rgba?\([0-9.,% ]+\)|hsla?\([0-9.,% deg]+\)|transparent|currentColor|inherit)$' then
  ...
  on conflict (component_key, property) do update
  set value = excluded.value, updated_at = now();
  return jsonb_build_object('ok', true);
end; $$;

create or replace function public.admin_delete_component_design_property(
  p_pw text, p_component_key text, p_property text
) returns void language plpgsql security definer set search_path = public as $$
declare
  allowed constant text[] := array['color','foregroundColor','mutedColor','backgroundColor','fontFamily','fontSize','fontWeight','lineHeight','letterSpacing','padding','margin','borderRadius','borderColor','borderWidth','opacity','display'];
begin
  ...
  where cd.component_key = p_component_key and cd.property = p_property;
end; $$;

create or replace function public.admin_apply_component_design(
  p_pw text, p_component_key text, p_reset_properties text[], p_values jsonb
) returns void language plpgsql security definer set search_path = public as $$
declare
  v_property text;
  allowed constant text[] := array['color','foregroundColor','mutedColor','backgroundColor','fontFamily','fontSize','fontWeight','lineHeight','letterSpacing','padding','margin','borderRadius','borderColor','borderWidth','opacity','display'];
...
revoke all on function public.admin_save_component_design(text, text, jsonb) from public;
