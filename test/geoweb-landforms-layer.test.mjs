...
test('landform data includes representative school geography and rivers stay as flow lines', async () => {
  ...
  for (const name of ['몽골고원', '콜로라도고원', '알티플라노고원', '만주평원', '팜파스', '야노스', '그란차코', '아라비아 사막', '타르 사막', '타클라마칸 사막', '파타고니아 사막', '그레이트플레인스', '중앙아시아 평원·저지', '중앙아시아 사막권', '오스트레일리아 사막권']) {
  ...
  assert.ok(category('deserts').some((feature) => feature.properties.nameKo === '파타고니아 사막' && feature.properties.sourceName === 'PATAGONIA' && feature.properties.sourceClass === 'Geoarea'), 'Patagonian Desert teaching extent retains the Patagonia source area');
...
test('GeoWeb draws selected terrain categories at 70 percent opacity over climate without replacing it', async () => {
  ...
  assert.match(source, /if\(!current\|\|geoArea\(feature\)>geoArea\(current\)\)labels\.set\(key,feature\)/);
